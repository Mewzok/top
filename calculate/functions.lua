...
-------------------魔剑
	...
	if math.mod(now_tick, 6) == 0 and now_tick > 0 then
		TakePetEndure( role , now_tick)	
		TakeUreItemHTXD (role,now_tick)		
	end
	if math.mod(now_tick, 30) == 0 and now_tick > 0 then  --2008万圣节亡灵及南瓜面具   面具在我头耐久30秒1扣
		WLface(role,now_tick)             --2008万圣节亡灵及南瓜面具   面具在我头 不buff5秒1刷	
	end
	if math.mod(now_tick, 12) == 0 and now_tick > 0 then
		NewMGH(role,now_tick)
	end
	-- if math.mod(now_tick, 30) == 0 and now_tick > 0 then--每30秒次判断一次
...
function HeiLong(role,now_tick)
	...
	if heilong == 5872 then 
		local sencend_30 = 30
		if math.mod(now_tick, sencend_30) == 0 and now_tick > 0 then
			local hp = GetChaAttr( role , ATTR_HP )
			local mxhp = GetChaAttr( role , ATTR_MXHP) 
...
function GiveJLCoins(role,elf,num1)
	...
	if lv >=JLMINLV and lv <JLMAXLV then 
		...
		num=num+1
		if math.mod( num , 1) == 0 and ure>=50 and is2==1 then
			GiveItemX ( role , 0 , 855 , num1 , 4 )										-------成长1点给一精灵硬币
		end
		if math.mod( num , 2) == 0 and ure>=50 and is2~=1 then
			GiveItemX ( role , 0 , 855 , num1 , 4 )										-------成长2点给一精灵硬币
		end
		-- if math.mod( num , 30) == 0  and ure>=50 and is2==1 then			-------成长30点给一精灵辉印一个
			-- GiveItemX ( role , 0 , 2588 , 1 , 4 )						
		-- end
		-- if math.mod( num , 60) == 0  and ure>=50 and is2~=1 then			-------成长60点给一精灵辉印一个
			-- GiveItemX ( role , 0 , 2588 , 1 , 4 )						
		-- end
		-- if math.mod( num , 120) == 0  and ure>=50  then						-------成长120点给一精灵辉印一个
			-- GiveItemX ( role , 0 , 2588 , 1 , 4 )						
		-- end
		-- if math.mod( num , 1200) == 0  and ure>=50 then						-------成长1200点给精灵王辉印一个
...
function MoJianBag(role,now_tick)--------------魔剑在我包
	...
	if math.mod( b , 180 ) == 0 and b < 1440 and b > 0 then 
...
function MoJianHand(role,now_tick)-----------------------魔剑在我手
	...
	if weapon_id == 6347 then
		...
		if math.mod( a , 180 ) == 0 and a < 1440 and a > 0 then
			CALCULATE_FUNCTIONS_LUA_000124 = GetResString("CALCULATE_FUNCTIONS_LUA_000124")
			CALCULATE_FUNCTIONS_LUA_000125 = GetResString("CALCULATE_FUNCTIONS_LUA_000125")
...
function Lvup_Str_2 ( role , Item_Num ,Item_Traget ) 
	local attr_type =  ITEMATTR_VAL_STR
	ElfLVUPIMP(role,Item_Traget,attr_type,1,0) 
end 
...
--Max Pet For Normal and Great Fruits
...
--Max Pet For Improved Fruits
function ElfLVUPIMP(role,item,attrType,lvupType,falseType)
	local elfLV=GetElfLV(item)
	local elfMaxEXP = GetItemAttr(item,ITEMATTR_MAXENERGY) 
	local elfMaxURE = GetItemAttr(item,ITEMATTR_MAXURE)
	local attrNum = GetItemAttr( item ,attrType )
...
function ZTLBalance()
	local nowTime = GetNowTimeW()
	if math.mod(nowTime,600)<5 then 
		local i,v
		local n = 0 
		local x = 0 
