...
--			SystemNotice( role , "ItemCount[i] = "..ItemCount[i])
	...
	if ItemID2 == 885 then 
		if Item2_Lv>=9 or Item3_Lv >= 9 then 
			SystemNotice ( role , "Refining Gem can improved to level 9 at most.")
			return 0
		end
	end	
...
function can_beuplv_item_main ( Table )
	...
	if Item_beuplv_ID < 5001 or Item_beuplv_ID > 10000 then					--判断融合道具是否为融合类型
...
function can_manufacture_item (...)
	...
	if arg.n ~= Length then
		CALCULATE_FORGE_LUA_000014 = GetResString("CALCULATE_FORGE_LUA_000014")
		Notice(CALCULATE_FORGE_LUA_000014..arg.n)
		return 0
	end	
	local Check = 0
--	SystemNotice( arg[1] , "调用合成检测主函数")
...
function GetChaName_born2 ( role )
	...
	local op1 = CheckBagItem( role, 128 )
	local op2 = CheckBagItem( role, 131 )
	local op3 = CheckBagItem( role, 134 )
	local op4 = CheckBagItem( role, 138 )
	if op1 < 1 and op2 < 1 and op3 < 1 and op4 < 1 then
		SystemNotice( role ,"You can only take the 2nd rebirth after the 1st rebirth.")
		return 0
	end
	local item_empty=IsEquip(role)
	if item_empty==LUA_TRUE then
